  ...
  useEffect(() => {
    ...
    const estacionesConAltaDemanda = rutaOptima.path.filter((estacion) => {
      ...
      const estacionCerrada = estacionesCerradas.find(
        (e) => e.id === estacionId && e.razon === "Alta actividad de reportes"
      );
            ...
            {rutaAlternativa && (
              <>
                ...
                <FlatList
                  ...
                  renderItem={({ item, index }) => (
                    <View style={[styles.stepCard, styles.alternativeCard]}>
                      <View style={styles.stepRow}>
                        <Text style={styles.stepText}>
                          {index + 1}. {item.nombre} - {item.linea}
                        </Text>
                  ...
                  ItemSeparatorComponent={() => (
                    <View style={styles.separator}>
                      <Feather name="arrow-down" size={24} color="#4CAF50" />
                    </View>
                  )}
                  contentContainerStyle={{ paddingBottom: 20 }}
                />
...
const styles = StyleSheet.create({
  ...
  stationButtonActive: {
    backgroundColor: "rgba(230, 128, 89, 0.05)",
    borderColor: "#E68059",
  },
  stationButtonIcon: {
    width: 40,
